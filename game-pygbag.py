    ...
    def load_data(self):
        if Game.WEB:
            ...
            if floor_value is not None:
                self.game_data["floor"] = int(floor_value)

            klayout_value = window.localStorage.getItem("keyboard_layout")
            if klayout_value is not None:
                self.game_data["keyboard_layout"] = str(klayout_value)
    ...
    def tick(self):
        ...
        for event in events:
            if event.type == pygame.QUIT:
                ...
                # Exit game
                ########################
                if event.key == K_ESCAPE and not Game.WEB:
                    self.running = False

                ########################
                # Fullscreen!
                ########################
                if event.key == K_F11 and not Game.WEB:
